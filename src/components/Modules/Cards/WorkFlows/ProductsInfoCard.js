...
export default function ProductsInfoCard(props) {

    return (
        <div style={{ boxShadow: `2px 2px 4px #059669` }} className={`p-5 bg-[var(--colBg)] shadow-lg rounded-2xl gap-5 grid grid-cols-5 cursor-pointer hover:py-7 transition-all duration-500`}>
            ...
            <div className={`border-l-2 border-[white] gap-2 flex flex-col justify-center-center`}>
                {props.name}
                <div className='text-[12px] text-[var(--colTextB)]'>سایز : {props.size}</div>
            </div>
