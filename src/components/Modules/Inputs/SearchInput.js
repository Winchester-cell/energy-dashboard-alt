...
export default function SearchInput({stnWidth = 250}) {
    ...
    return (
        <div style={{width: extended ? stnWidth : 54}} onClick={!extended ? () => setExtended(true) : null} className={`flex ${extended ? '' : 'cursor-pointer'} transition-all duration-500 items-center bg-[var(--colBg)] px-5 py-2 gap-3 rounded-full`}>
